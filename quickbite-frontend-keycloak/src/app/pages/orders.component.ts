    ...
    <div class="container">
      <div class="card">
        ...
        <div style="display:flex;gap:10px;align-items:end;flex-wrap:wrap;margin-bottom:16px">
          <div style="flex:2;min-width:160px"><label>Plat</label><input [(ngModel)]="item" placeholder="Ex. Pizza"/></div>
          <div style="flex:1;min-width:100px"><label>Prix (€)</label><input type="number" [(ngModel)]="price"/></div>
          ...
          <table>
            ...
            <tbody>
              @for (o of orders(); track o.id) {
                <tr>
                  <td>{{ o.id }}</td><td>{{ o.item }}</td><td>{{ o.price | number:'1.2-2' }} €</td>
                  <td style="text-align:right">
                    @if (isAdmin) { <button class="secondary" (click)="remove(o.id)">Supprimer</button> }
                  </td>
                </tr>
              }
            </tbody>
        ...
        <p class="muted" style="margin-top:14px">Suppression réservée au rôle <b>admin</b> (l'API renvoie 403 sinon).</p>
      </div>
    </div>
  `,
})
export class OrdersComponent implements OnInit {
  ...
  add(): void {
    ...
    this.api.createOrder(this.item, this.price).subscribe({
      next: () => { this.item = ''; this.price = 0; this.load(); },
